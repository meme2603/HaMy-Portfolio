...
/* ============================================================
   HIỆU ỨNG REVEAL (HIỆN ẢNH KHI CUỘN) - DÙNG CHO TRANG CHI TIẾT
   ============================================================ */
function handleScrollReveal() {
    const images = document.querySelectorAll('.reveal-img');
    
    // Nếu trong trang khum có cái ảnh nào class là reveal-img thì nghỉ, khum chạy nữa
    if (images.length === 0) return;

    images.forEach(img => {
        const rect = img.getBoundingClientRect();
        // Kiểm tra xem ảnh đã lọt vào tầm mắt chưa
...
// Chạy hàm này mỗi khi bà cuộn chuột
window.addEventListener('scroll', handleScrollReveal);

// Chạy thử 1 lần ngay khi vừa load trang (phòng trường hợp ảnh nằm ngay đầu trang)
